    ...
    internal sealed class ConsistencyWriter
    {
            ...
            bool forceRefresh)
        {
            ...
            if (request.RequestContext.GlobalStrongWriteStoreResult == null)
            {
                ...
                Tuple<PerProtocolPartitionAddressInformation, AddressCacheToken> addressInfo = await this.addressSelector.ResolveAddressesAsync(request, forceRefresh);
                ...
                request.RequestContext.ClientRequestStatistics.ContactedReplicas = addressInfo.Item1.ReplicaUris.ToList();

                Uri primaryUri = addressInfo.Item1.GetPrimaryUri(request);
                this.LastWriteAddress = primaryUri.ToString();

                this.connectionStateListener?.UpdateConnectionState(primaryUri, addressInfo.Item2);
